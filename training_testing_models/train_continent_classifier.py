...
# Globals
images_sorted_by_continent_folder = "data/images_sorted_by_continent"
continent_classifier_model_folder = "models/continent_classifier_model"
resize_size = (150, 150)
test_size = 0.3
batch_size = 128

# (250, 250) ContinentClassifierModel
class ContinentClassifierModel:

    def __init__(self, num_continents, input_shape=(150, 150, 3)):
        ...
        self.model.add(layers.Dense(units=self.num_continents, activation='softmax'))
        assert self.model.output_shape == (None, num_continents)
...
def train():
    # New Model
    cnn = ContinentClassifierModel(num_continents=len(glob.glob(images_sorted_by_continent_folder + "/*")))

    # Load Dataset
    dataset_train = image_dataset_from_directory(images_sorted_by_continent_folder,
    ...
    dataset_test = image_dataset_from_directory(images_sorted_by_continent_folder,
                                                label_mode="categorical",
                                                image_size=resize_size,
                                                batch_size=batch_size,
                                                seed=42,
    ...
    class_weights = get_class_weights(images_sorted_by_continent_folder)
    print(class_weights)
    ...
    cnn.model.fit(
        ...
        verbose = 1,
        callbacks = [ModelCheckpoint(continent_classifier_model_folder + "/continent_classifier_250_250.h5", save_best_only=True, save_weights_only=False, monitor='val_loss', mode='min')],
...
def test():
    # Load Model
    cnn = ContinentClassifierModel(num_countries=len(glob.glob(images_sorted_by_continent_folder + "/*")))
    cnn.load_model(continent_classifier_model_folder + "/continent_classifier_250_250.h5")
    # Plot Confusion Matrix
    test_dataset = image_dataset_from_directory(images_sorted_by_continent_folder,
                                                label_mode="categorical",
                                                image_size=resize_size,
                                                batch_size=batch_size,
                                                seed=42,
